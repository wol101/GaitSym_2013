...
void TwoCylinderWrapStrap::SetCylinder1Position(dReal x, dReal y, dReal z)
{
...
void TwoCylinderWrapStrap::GetCylinder1(Body **body, dVector3 position, dReal *radius, dQuaternion q)
{
    *body = m_CylinderBody;
...
void TwoCylinderWrapStrap::SetCylinderAxis(dReal x, dReal y, dReal z)
{
...
void TwoCylinderWrapStrap::SetCylinderQuaternion(dReal q0, dReal q1, dReal q2, dReal q3)
{
...
void TwoCylinderWrapStrap::SetCylinder2Position(dReal x, dReal y, dReal z)
{
...
void TwoCylinderWrapStrap::GetCylinder2(Body **body, dVector3 position, dReal *radius, dQuaternion q)
{
    *body = m_CylinderBody;
...
void TwoCylinderWrapStrap::Calculate(dReal deltaT)
{
    int i;
    ...
    dReal tension = 1; // normalised initially because tension is applied by muscle
...
void TwoCylinderWrapStrap::TwoCylinderWrap(pgd::Vector &origin, pgd::Vector &insertion, pgd::Vector &cylinderPosition1, dReal radius1,
                                           ...
                                           pgd::Vector *pathCoordinates, int *numPathCoordinates, int *wrapOK)
{
    ...
    *numPathCoordinates = 0;
    ...
    dReal cyl1_start_angle = atan2(E2.y - C.y, E2.x - C.x);
    dReal cyl1_end_angle = atan2(F2.y - C.y, F2.x - C.x);
    dReal cyl1_theta = cyl1_end_angle - cyl1_start_angle;
    if (cyl1_theta < 0) cyl1_theta = cyl1_theta + 2 * M_PI;
    dReal cyl2_start_angle = atan2(G2.y - D.y, G2.x - D.x);
    dReal cyl2_end_angle = atan2(H1.y - D.y, H1.x - D.x);
    dReal cyl2_theta = cyl2_end_angle - cyl2_start_angle;
    ...
    dReal l1, l2, l3, l4, l5;
    dReal planar_path_length, delta_Z, cyl1_del_theta, cyl2_del_theta, theta;
    int i, j;
        ...
        *pathLength = sqrt(delta_Z * delta_Z + planar_path_length * planar_path_length);
        ...
        if (pathCoordinates)
        {
            ...
            for (j = 1; j <= nPointsPerCylinderArc - 1; j++)
            {
                ...
                pathCoordinates[i].z = O.z + delta_Z * (l1 + ((dReal)j / (dReal)nPointsPerCylinderArc) * l2) / planar_path_length;
                i = i + 1;
            }
            ...
            for (j = 1; j <= nPointsPerCylinderArc - 1; j++)
            {
                ...
                pathCoordinates[i].z = O.z + delta_Z * (l1 + l2 + l3 + ((dReal)j / (dReal)nPointsPerCylinderArc) * l4) / planar_path_length;
                i = i + 1;
            }
...
void TwoCylinderWrapStrap::FindCircleCircleTangents(pgd::Vector &c1, dReal radius1, pgd::Vector &c2, dReal radius2,
                                                    ...
                                                    pgd::Vector &inner1_p1, pgd::Vector &inner1_p2, pgd::Vector &inner2_p1, pgd::Vector &inner2_p2, int *number_of_tangents)
{
    ...
    // ***************************
    dReal radius2a = radius2 - radius1;
    ...
    dReal v1x = -(outer1_p2.y - c1.y);
    dReal v1y = outer1_p2.x - c1.x;
    dReal v1_length = sqrt(v1x * v1x + v1y * v1y);
    ...
    dReal v2x = outer2_p2.y - c1.y;
    dReal v2y = -(outer2_p2.x - c1.x);
    dReal v2_length = sqrt(v2x * v2x + v2y * v2y);
    ...
    // ***************************
    dReal radius1a = radius1 + radius2;
...
void TwoCylinderWrapStrap::FindTangents(pgd::Vector &center, dReal radius, pgd::Vector &external_point, pgd::Vector &pt1, pgd::Vector &pt2, int *number_of_tangents)
{
    // Find the distance squared from the
    // external point to the circle's center.
    dReal dx = center.x - external_point.x;
    dReal dy = center.y - external_point.y;
    dReal D_squared = dx * dx + dy * dy;
    ...
    dReal L = sqrt(D_squared - radius * radius);
...
void TwoCylinderWrapStrap::FindCircleCircleIntersections(dReal cx0, dReal cy0, dReal radius0, dReal cx1, dReal cy1, dReal radius1,
                                                         pgd::Vector &intersection1, pgd::Vector &intersection2, int *number_of_intersections)
{
    ...
    // Find a and h.
    dReal a = (radius0 * radius0 - radius1 * radius1 + dist * dist) / (2 * dist);
    dReal h = sqrt(radius0 * radius0 - a * a);

    // Find P2.
    dReal cx2 = cx0 + a * (cx1 - cx0) / dist;
    dReal cy2 = cy0 + a * (cy1 - cy0) / dist;
...
dReal TwoCylinderWrapStrap::vector_distance2d(pgd::Vector &v1, pgd::Vector v2)
{
    return sqrt((v2.x - v1.x) * (v2.x - v1.x) + (v2.y - v1.y) * (v2.y - v1.y));
}

// calculate the 3D length of a vector
dReal TwoCylinderWrapStrap::vector_distance3d(pgd::Vector &v1, pgd::Vector &v2)
{
    return sqrt((v2.x - v1.x) * (v2.x - v1.x) + (v2.y - v1.y) * (v2.y - v1.y) + (v2.z - v1.z) * (v2.z - v1.z));
}

// return the vector in the direction of v1 to v2 with magnitude specified
void TwoCylinderWrapStrap::vector_with_magnitude(pgd::Vector &v1, pgd::Vector &v2, dReal magnitude, pgd::Vector &v)
{
    pgd::Vector del_v = v2 - v1;
    dReal mag = vector_distance3d(v1, v2);
