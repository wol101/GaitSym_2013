...
void BoxCarDriver::SetBoxCarParameters(dReal CycleTime, dReal Delay, dReal Width, dReal Height)
{
    RANGE(Delay, 0, 1);
    RANGE(Width, 0, 1);
...
dReal BoxCarDriver::GetValue(dReal Time)
{
    // get a normalised cycle time (value from 0 to 1)
    dReal NormalisedCycleTime = fmod(Time, m_CycleTime) / m_CycleTime;

    dReal Output = 0;
    dReal OffTime = m_Delay + m_Width;
