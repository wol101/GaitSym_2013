...
static dReal CalculateForceError (dReal lce, void *params);
static dReal zeroin(dReal *ax, dReal *bx, dReal (*f)(dReal x, void *info), void *info, dReal *tol);
...
void MAMuscleComplete::SetSerialElasticProperties(dReal serialStrainAtFmax, dReal serialStrainRateAtFmax, dReal tendonLength, MAMuscleComplete::StrainModel serialStrainModel)
{
    dReal serialDampingConstant, serialElasticConstant;
...
void MAMuscleComplete::SetParallelElasticProperties(dReal parallelStrainAtFmax, dReal parallelStrainRateAtFmax, dReal parallelElementLength, MAMuscleComplete::StrainModel parallelStrainModel)
{
    dReal parallelDampingConstant, parallelElasticConstant;
...
void MAMuscleComplete::SetMuscleProperties(dReal vMax, dReal Fmax, dReal K, dReal Width)
{
...
void MAMuscleComplete::SetActivationKinetics(bool activationKinetics,
                                             dReal akFastTwitchProportion,
                                             dReal akTActivationA, dReal akTActivationB,
                                             dReal akTDeactivationA, dReal akTDeactivationB)
{
    m_ActivationKinetics = activationKinetics;

    // original code
    //dReal ft = 0.5; // arbitrary set activation kinetics as 50% fast twitch
    //dReal tact = 80e-3 - 0.47e-3 * ft; // Umberger et al 2003 eq 4
    //dReal tdeact = 90e-3 - 0.56e-3 * ft; // Umberger et al 2003 eq 4
...
void MAMuscleComplete::LateInitialisation()
{
    ...
    dReal minlpe = m_Params.spe - (m_Params.spe * m_Params.width / 2);
    if (minlpe < 0) minlpe = 0;
    dReal maxlpe = m_Params.len - m_Params.sse; // this would be right with no damping
...
void MAMuscleComplete::SetActivation(dReal activation, dReal timeIncrement)
{
/*
...
*/
    ...
    if (m_ActivationKinetics || m_ActivationRate != 0)
    {
        ...
        else
        {
            if (m_ActivationKinetics)
            {
                ...
                dReal qdot = (m_Stim - m_Params.alpha) * (t1 * m_Stim + t2);
            ...
            {
                dReal delAct = m_ActivationRate * m_Params.timeIncrement;
    ...
    else
    {
        ...
        else
        {
            ...
            {
                dReal ax, bx, r, tol;
                // dReal range = maxlpe - minlpe; // this doesn't quite work because of damping
                dReal range = m_Params.len; // this should be bigger than necessary
                int nInc = 100;
                dReal inc = range / nInc;
                dReal high_target, low_target, err;
                int i;
                for (i = 1; i <= nInc; i++)
                {
...
dReal MAMuscleComplete::GetMetabolicPower()
{
    ...
    dReal power = m_Params.alpha * m_Params.f0 * m_Params.vmax * sigma;
...
dReal MAMuscleComplete::GetESE() // energy serial element
{
    if (m_Params.lse <= m_Params.sse) return 0;

    dReal energy;
    dReal extension = m_Params.lse - m_Params.sse;
...
// note that with damping this is the energy stored but not the energy that will be returned
dReal MAMuscleComplete::GetEPE() // energy serial element
{
    if (m_Params.lpe <= m_Params.spe) return 0;

    dReal energy;
    dReal extension = m_Params.lpe - m_Params.spe;
    // serial element model
    switch (m_Params.smpe)
    {
    case MAMuscleComplete::linear:
...
dReal CalculateForceError (dReal lce, void *params)
{
...
dReal zeroin(dReal *ax, dReal *bx, dReal (*f)(dReal x, void *info), void *info, dReal *tol)
{
    /* System generated locals */
    dReal ret_val = 0, d__1, d__2;

    /* Local variables */
    dReal a, b, c__, d__, e, p, q, r__, s, fa, fb, fc, xm, eps, tol1;
