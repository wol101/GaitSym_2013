...
dReal DataTargetScalar::GetMatchValue()
{
    ...
    else if ((positionReporter = dynamic_cast<PositionReporter *>(m_Target)) != 0)
    {
        ...
        case Q0:
            ...
            matchScore = m_Weight - m_Slope * PositiveFunction(pq.n - m_ValueList[m_TargetTimeListIndex]);
            break;
        case Q1:
            pq = positionReporter->GetWorldQuaternion();
            matchScore = m_Weight - m_Slope * PositiveFunction(pq.v.x - m_ValueList[m_TargetTimeListIndex]);
            break;
        case Q2:
            pq = positionReporter->GetWorldQuaternion();
            matchScore = m_Weight - m_Slope * PositiveFunction(pq.v.y - m_ValueList[m_TargetTimeListIndex]);
            break;
        case Q3:
            pq = positionReporter->GetWorldQuaternion();
            matchScore = m_Weight - m_Slope * PositiveFunction(pq.v.z - m_ValueList[m_TargetTimeListIndex]);
            break;
        case XP:
            pv = positionReporter->GetWorldPosition();
            matchScore = m_Weight - m_Slope * PositiveFunction(pv.x - m_ValueList[m_TargetTimeListIndex]);
            break;
        case YP:
            pv = positionReporter->GetWorldPosition();
            matchScore = m_Weight - m_Slope * PositiveFunction(pv.y - m_ValueList[m_TargetTimeListIndex]);
            break;
        case ZP:
            pv = positionReporter->GetWorldPosition();
            matchScore = m_Weight - m_Slope * PositiveFunction(pv.z - m_ValueList[m_TargetTimeListIndex]);
            break;
        case XV:
