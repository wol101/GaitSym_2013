...
dReal DataTargetVector::GetMatchValue()
{
    ...
    else if ((positionReporter = dynamic_cast<PositionReporter *>(m_Target)) != 0)
    {
        ...
        err = (vec - m_ValueList[m_TargetTimeListIndex]).Magnitude2();
        if (m_MatchType == linear) err = sqrt(err);
...
void DataTargetVector::Draw()
{
    ...
    // and draw the sphere
    m_PhysRep->SetDisplayPosition(m_ValueList[m_TargetTimeListIndex].x, m_ValueList[m_TargetTimeListIndex].y, m_ValueList[m_TargetTimeListIndex].z);
    m_PhysRep->Draw();
}
#endif
