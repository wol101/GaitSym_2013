...
void StackedBoxCarDriver::SetCycleTimes(dReal *CycleTimes)
{
    for (int i = 0; i < m_StackSize; i++) m_CycleTimes[i] = CycleTimes[i];
}

void StackedBoxCarDriver::SetDelays(dReal *Delays)
{
    for (int i = 0; i < m_StackSize; i++) m_Delays[i] = Delays[i];
}

void StackedBoxCarDriver::SetWidths(dReal *Widths)
{
    for (int i = 0; i < m_StackSize; i++) m_Widths[i] = Widths[i];
}

void StackedBoxCarDriver::SetHeights(dReal *Heights)
{
    for (int i = 0; i < m_StackSize; i++) m_Heights[i] = Heights[i];
}


dReal StackedBoxCarDriver::GetValue(dReal Time)
{
    dReal Output = 0;
    dReal NormalisedCycleTime;
    dReal OffTime;
