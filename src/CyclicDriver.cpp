...
void CyclicDriver::SetValueDurationPairs(int size, dReal *valueDurationPairs)
{
    ...
    if (m_ListLength != size / 2)
    {
        ...
        if (m_ValueList) delete [] m_ValueList;
        m_ListLength = size / 2;
        m_DurationList = new dReal[m_ListLength + 1]; // this is +1 because the first entry is zero
        m_ValueList = new dReal[m_ListLength + 1]; // this lets the interp version wrap properly
    }
    m_DurationList[0] = 0;
