...
*/
    ...
    else if (strcmp((const char *)buf, "Slider") == 0)
    {
        ...
        dBodyID body2ID = 0;
        if (strcmp((const char *)buf, "World")) THROWIFZERO(body2ID = m_BodyList[(const char *)buf]->GetBodyID());
        sliderJoint->Attach(body1ID, body2ID);
...
void Simulation::ParseMuscle(xmlNodePtr cur)
{
    ...
    else if (strcmp((const char *)buf, "MinettiAlexanderComplete") == 0)
    {
        ...
        dReal serialStrainRateAtFmax = Util::Double(buf);
        ...
        dReal parallelStrainRateAtFmax = Util::Double(buf);
...
void Simulation::ParseDriver(xmlNodePtr cur)
{
    ...
    else if (strcmp((const char *)buf, "StackedBoxCar") == 0)
    {
        ...
        // assumes all cycles are the same duration
        std::vector<dReal> *CycleTimes = StackedBoxCarDriver1->GetCycleTimes();
...
void Simulation::ParseDataTarget(xmlNodePtr cur)
{
    ...
    else if (strcmp((const char *)buf, "Vector") == 0)
    {
        ...
        Colour dataTargetColour = m_Interface.ReporterColour;
        dataTargetColour.alpha *= 0.5;
        dataTargetVector->SetColour(dataTargetColour);
        dataTargetVector->SetRadius(m_Interface.ReporterRadius);
#endif
...
void Simulation::OutputProgramState()
{
    ...
    for (iter0 = m_TagContentsList.begin(); iter0 != m_TagContentsList.end(); iter0++)
    {
        ...
        if ((!xmlStrcmp(cur->name, (const xmlChar *)"JOINT")))
        {
            ...
            if (jp)
            {
                dBodyID bodyID = jp->GetBody(0);
                Body *body;
                if (bodyID)
                {
                    body = (Body *)(dBodyGetData(jp->GetBody(0)));
                    jp->GetHingeAnchor(v);
            ...
            if (jps)
            {
                dBodyID bodyID = jps->GetBody(0);
                Body *body;
                if (bodyID)
                {
                    body = (Body *)(dBodyGetData(jps->GetBody(0)));
                    jps->GetSliderAxis(v);
                    if (m_ModelStateRelative)
                    {
                        dBodyVectorFromWorld(jps->GetBody(0), v[0], v[1], v[2], result);
                        sprintf(m_Buffer, "%s %.17e %.17e %.17e", body->GetName()->c_str(), result[0], result[1], result[2]);
                    }
                    else
                    {
