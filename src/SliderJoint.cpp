...
void SliderJoint::SetSliderAxis(dReal x, dReal y, dReal z)
{
    dVector3 v;
    v[0] = x; v[1] = y; v[2] = z;
    dNormalize3(v);
...
dReal SliderJoint::GetSliderDistance()
{
    return dJointGetSliderPosition(m_JointID) + m_StartDistanceReference;
}

dReal SliderJoint::GetSliderDistanceRate()
{
    return dJointGetSliderPositionRate(m_JointID);
}

void SliderJoint::SetStartDistanceReference(dReal startDistanceReference)
{
    m_StartDistanceReference = startDistanceReference;
}

void SliderJoint::SetJointStops(dReal loStop, dReal hiStop)
{
...
void SliderJoint::SetStopCFM(dReal cfm)
{
    dJointSetSliderParam (m_JointID, dParamStopCFM, cfm);
}

void SliderJoint::SetStopERP(dReal erp)
{
    dJointSetSliderParam (m_JointID, dParamStopERP, erp);
}

void SliderJoint::SetStopSpringDamp(dReal springConstant, dReal dampingConstant, dReal integrationStep)
{
...
void SliderJoint::SetStopSpringERP(dReal springConstant, dReal ERP, dReal integrationStep)
{
...
void SliderJoint::SetStopBounce(dReal bounce)
{
...
void SliderJoint::Draw()
{
    ...
    if (gAxisFlag)
    {
        ...
        dBodyID b0= GetBody(0);
        dBodyID b1= GetBody(1);

        if (b0 == 0 && b1 == 0) return;

        dReal length = GetSliderDistance();
