...
void MainWindow::open()
{
    ...
    QString fileName = QFileDialog::getOpenFileName(this, tr("Open Config File"), info.absoluteFilePath(), tr("Config Files (*.xml)"));
    ...
    if (fileName.isNull() == false)
    {
        ...
        strcpy(readFileName, fileName.toLatin1());
        ...
        strcpy(graphicsRoot, graphicsPath.toLatin1());
...
void MainWindow::restart()
{
    ...
    strcpy(readFileName, configFile.absoluteFilePath().toLatin1());
    gConfigFilenamePtr = readFileName;
...
void MainWindow::saveas()
{
    ...
    QString fileName = QFileDialog::getSaveFileName(this, tr("Save Model State File (Relative)"), info.absolutePath(), tr("Config Files (*.xml)"));

    if (fileName.isNull() == false)
    {
        gSimulation->SetModelStateRelative(true);
        gSimulation->SetOutputModelStateFile(fileName.toLatin1());
...
void MainWindow::saveasworld()
{
    ...
    QString fileName = QFileDialog::getSaveFileName(this, tr("Save Model State File (World)"), info.absolutePath(), tr("Config Files (*.xml)"));
        ...
        if ((stepCount % skip) == 0)
        {
            if (trackingFlag)
            {
                ...
                if (body)
                {
                    ...
                    glWidget->SetCameraCOIX(position[0] + trackingOffset);
...
void MainWindow::listMuscleChecked(QListWidgetItem* item)
{
    ...
    (*gSimulation->GetMuscleList())[std::string(item->text().toLatin1())]->SetAllVisible(visible);
    updateGL();
}
...
void MainWindow::listBodyChecked(QListWidgetItem* item)
{
    bool visible = true;
    if (item->checkState() == Qt::Unchecked) visible = false;
    (*gSimulation->GetBodyList())[std::string(item->text().toLatin1())]->SetVisible(visible);
    updateGL();
}
...
void MainWindow::listJointChecked(QListWidgetItem* item)
{
    bool visible = true;
    if (item->checkState() == Qt::Unchecked) visible = false;
    (*gSimulation->GetJointList())[std::string(item->text().toLatin1())]->SetVisible(visible);
    updateGL();
}
...
void MainWindow::listGeomChecked(QListWidgetItem* item)
{
   bool visible = true;
    if (item->checkState() == Qt::Unchecked) visible = false;
    (*gSimulation->GetGeomList())[std::string(item->text().toLatin1())]->SetVisible(visible);
    updateGL();
}
...
void MainWindow::menuOutputs()
{
    ...
    if (status == QDialog::Accepted) // write the new settings
    {
        for (i = 0; i < dialogUI->listWidgetBody->count(); i++)
        {
            ...
            (*gSimulation->GetBodyList())[std::string(item->text().toLatin1())]->SetDump(dump);
        }
        ...
        for (i = 0; i < dialogUI->listWidgetMuscle->count(); i++)
        {
            ...
            if (item->checkState() == Qt::Unchecked) dump = false; else dump = true;
            (*gSimulation->GetMuscleList())[std::string(item->text().toLatin1())]->SetDump(dump);
            (*gSimulation->GetMuscleList())[std::string(item->text().toLatin1())]->GetStrap()->SetDump(dump);
        }
        ...
        for (i = 0; i < dialogUI->listWidgetGeom->count(); i++)
        {
            ...
            if (item->checkState() == Qt::Unchecked) dump = false; else dump = true;
            (*gSimulation->GetGeomList())[std::string(item->text().toLatin1())]->SetDump(dump);
        }
        ...
        for (i = 0; i < dialogUI->listWidgetJoint->count(); i++)
        {
            ...
            if (item->checkState() == Qt::Unchecked) dump = false; else dump = true;
            (*gSimulation->GetJointList())[std::string(item->text().toLatin1())]->SetDump(dump);
        }
        ...
        for (i = 0; i < dialogUI->listWidgetDriver->count(); i++)
        {
            ...
            if (item->checkState() == Qt::Unchecked) dump = false; else dump = true;
            (*gSimulation->GetDriverList())[std::string(item->text().toLatin1())]->SetDump(dump);
        }
        ...
        for (i = 0; i < dialogUI->listWidgetDataTarget->count(); i++)
        {
            ...
            if (item->checkState() == Qt::Unchecked) dump = false; else dump = true;
            (*gSimulation->GetDataTargetList())[std::string(item->text().toLatin1())]->SetDump(dump);
        }
        ...
        for (i = 0; i < dialogUI->listWidgetReporter->count(); i++)
        {
            ...
            if (item->checkState() == Qt::Unchecked) dump = false; else dump = true;
            (*gSimulation->GetReporterList())[std::string(item->text().toLatin1())]->SetDump(dump);
        }
...
void MainWindow::menuSaveDefaultView()
{
    ...
    settings->setValue("TrackingOffset", ui->lineEditTrackingOffset->text().toDouble());
